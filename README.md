...
Run `pnpm drafts:automation-service -- --port 3030 --api-key automation-secret` to start a local automation endpoint for those work orders. By default it stores a local handoff record at `/:platform/drafts/:automationDraftId`; pass `--handler-module ./path/to/handler.mjs` when a real Playwright or official-API implementation should create the platform draft and return the real draft id/URL. Handler modules receive the selected platform's `DRAFT_AUTOMATION_<PLATFORM>_ACCESS_TOKEN`, `*_COOKIES`, `*_STORAGE_STATE_JSON`, `*_STORAGE_STATE_PATH`, and creator URL settings as `platformSession`; direct connector-upstream requests can also merge per-request forwarded credentials into that handler-only session. `/health`, `/contract`, outbox records, and converted work orders expose only redacted readiness/credential summaries, and the service does not persist those session secrets. Add `--require-session` or `DRAFT_AUTOMATION_<PLATFORM>_REQUIRE_SESSION=true` to reject work orders before handler execution when neither configured nor connector-forwarded login material is available. The service also accepts the connector upstream contract directly at `POST /:platform/drafts`, so `DRAFT_CONNECTOR_ZHIHU_UPSTREAM_DRAFT_ENDPOINT=http://localhost:3030/zhihu/drafts` can point connector forwarding at the automation handler without running the sandbox/runner layer. The built-in `scripts/handlers/playwright-draft-handler.mjs` can drive creator-center pages when Playwright is installed and `DRAFT_AUTOMATION_<PLATFORM>_PLAYWRIGHT_SELECTORS_JSON` or `*_SELECTORS_PATH` provides selectors for title, body, optional summary/tags, save-draft, and result URL extraction; run `pnpm drafts:check-playwright -- --platform zhihu --screenshot-dir .runtime/draft-playwright-checks` to verify the creator draft page, login session, and selectors without filling or saving a draft.
When the automation service health reports `needs_session`, the draft connector now marks that upstream as `needs_action`, and API preflight holds the affected real draft targets before they reach the worker queue.
Run `pnpm drafts:capture-playwright-session -- --platform zhihu --url <creator-draft-url> --wait-for-selector "<logged-in-selector>" --save-env` after logging in through the opened browser to save a Playwright storage state under `.runtime/draft-sessions` by default. With `--save-env`, the script updates `.env` with the creator draft URL, storage-state path, `DRAFT_AUTOMATION_<PLATFORM>_REQUIRE_SESSION=true`, and an optional selector-file path; it does not write raw cookies into `.env`.
Run `pnpm drafts:capture-playwright-selectors -- --platform zhihu --url <creator-draft-url> --fields title,body,saveDraft --save-env` with that saved session to click the real title editor, body editor, and save-draft button in the creator page. The script writes a handler-ready selector JSON file under `.runtime/draft-selectors` by default and updates `.env` with `DRAFT_AUTOMATION_<PLATFORM>_PLAYWRIGHT_SELECTORS_PATH`.
The Playwright capture, check, automation-service, and smoke scripts load the workspace `.env` by default, so the saved session and selector pointers flow into the next command without exporting secrets into the shell. Run `pnpm drafts:smoke-playwright -- --platform zhihu --headed` after `check-playwright` succeeds to fill and save a timestamped smoke draft through the built-in handler; the default save path now requires an explicit `saveDraft` selector, and `--no-click-save` is only for fill-only rehearsal.
Run `pnpm drafts:enable-playwright-automation -- --api-key automation-secret` after the smoke check to wire the local draft connector to `pnpm drafts:automation-service` for Zhihu, Bilibili, and Xiaohongshu. The script enables the three real draft targets, points `DRAFT_CONNECTOR_<PLATFORM>_UPSTREAM_DRAFT_ENDPOINT` at `http://localhost:3030/<platform>/drafts`, configures the Playwright handler module, and marks platform sessions as required.
...
pnpm drafts:capture-playwright-session -- --platform zhihu --url https://creator.example.com/drafts/new --wait-for-selector "#account-menu" --save-env
pnpm drafts:capture-playwright-selectors -- --platform zhihu --url https://creator.example.com/drafts/new --fields title,body,saveDraft --save-env
pnpm drafts:check-playwright -- --platform zhihu --screenshot-dir .runtime/draft-playwright-checks
pnpm drafts:smoke-playwright -- --platform zhihu --headed
pnpm drafts:enable-playwright-automation -- --api-key automation-secret
DRAFT_CONNECTOR_ZHIHU_UPSTREAM_DRAFT_ENDPOINT=http://localhost:3030/zhihu/drafts
DRAFT_CONNECTOR_ZHIHU_UPSTREAM_DRAFT_API_KEY=automation-secret
...
When a real draft target is not enabled, has no draft endpoint, uses an inferred local connector endpoint while that connector is offline, or declares an offline/not-ready upstream health endpoint, the API marks that target as needing manual action before it reaches the worker queue.
...
The draft connector test starts the built local draft connector and verifies workspace `.env` bootstrap, public-base runtime outbox links, the upstream contract endpoint, checker script, upstream sandbox script, automation service script, direct connector-upstream automation service ingestion, Playwright session capture, Playwright selector capture, Playwright setup checker, direct Playwright smoke draft creation, Playwright handler contract, Playwright save-draft selector guard, Playwright automation enablement script output, work-order runner script including automation endpoint delegation, async upstream callback handoff, and upstream proxy enablement script, local enablement script output, three-platform disabled/missing-credential/credential-forwarding-mismatch/offline-connector/explicit-local-endpoint/offline-upstream/upstream-missing-session preflight behavior, worker-side draft config drift manual-action handling, explicit local endpoint status inference, retry preflight queue protection, three-platform offline-upstream recovery after retry, connector readiness reporting, Zhihu/Bilibili/Xiaohongshu real-draft task results, idempotent connector draft reuse for repeated worker attempts, synchronous upstream draft forwarding, stale upstream forwarding reservation recovery, fresh in-flight reservation reuse without duplicate upstream calls, upstream rejection as manual action with local draft URLs, deduplicated manual status sync issues, status credential-forwarding mismatch sync protection, offline upstream status sync preflight, stale-link clearing on retry, and retry recovery, credential-gated upstream draft/status forwarding without outbox credential persistence, upstream status sync forwarding, browsable draft detail URLs, external draft URL handoff, outbox list entries, local outbox draft files, and manual status sync results when their connector endpoints are configured.
