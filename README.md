...
Run `pnpm drafts:upstream-sandbox -- --port 3020 --api-key sandbox-secret` to start a local upstream proxy sandbox that implements the same draft/status/health contract and stores accepted proxy drafts in `.runtime/draft-upstream-sandbox`.
