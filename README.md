...
Run `pnpm drafts:run-work-orders -- --sandbox-base-url http://localhost:3020 --api-key sandbox-secret --once` to execute those pending sandbox work orders with a local runner; it is a safe stand-in for the future Playwright/official-API automation worker and calls the same completion endpoint with generated creator-center draft links.
