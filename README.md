...
The API exposes connector readiness in `/runtime/status`, including each platform's `draftReady` flag and readiness issue codes, and the publish page shows whether each Zhihu/Bilibili/Xiaohongshu draft target is ready before creating connector-backed draft tasks.
