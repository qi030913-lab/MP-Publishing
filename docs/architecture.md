...
- `/publish/tasks/:taskId/sync` 当前支持公众号 `freepublish/get`、各平台可选 `*_STATUS_ENDPOINT`，以及本地 draft connector 可选 upstream status endpoint 手动状态同步；当连接器 status 上游离线或凭证转发配置不一致时会提前转待人工处理
