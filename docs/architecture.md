...
- `/runtime/status` 会同时返回 worker/queue/task 统计和 draft connector 就绪状态，用于发布页提示知乎 / B站 / 小红书草稿连接器是否在线
